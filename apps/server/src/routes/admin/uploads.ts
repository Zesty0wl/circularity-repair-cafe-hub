...
export async function adminUploadsRoutes(app: FastifyInstance): Promise<void> {
  app.post('/api/admin/uploads/branding', async (request, reply) => {
    ...
    const saved = await saveValidatedImage(buf, file.mimetype, 'branding', { maxLongestEdge: longest, quality: 0.85 });
  ...
  app.post('/api/admin/uploads/avatar/:userId', async (request, reply) => {
    ...
    const buf = await file.toBuffer();
    const saved = await saveValidatedImage(buf, file.mimetype, 'profiles', { maxLongestEdge: 600, quality: 0.85 });
